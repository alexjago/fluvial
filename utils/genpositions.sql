...
CREATE VIEW RailTrips AS SELECT DISTINCT trip_id, RailRoutes.route_id FROM Trips, RailRoutes
WHERE Trips.route_id = RailRoutes.route_id;

CREATE TABLE StopTimesT AS SELECT DISTINCT trip_id, stop_id, stop_sequence FROM StopTimes;

CREATE TABLE RailStopsT AS SELECT DISTINCT stop_id, stop_sequence, route_id FROM StopTimesT, RailTrips
WHERE StopTimesT.trip_id = RailTrips.trip_id;
...
FROM RailTripStops T, Routes R ON R.route_id = T.route_id;
